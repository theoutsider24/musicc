...
class MusiccContext:
    def __init__(
        ...
        self.mus_etree = etree.fromstring(self.mus_file,parser=parser)
        self.odr_etree = etree.fromstring(self.odr_file,parser=parser)
        self.osc_etree = etree.fromstring(self.osc_file,parser=parser)
        self.catalog_etrees = [
    ...
    def write_etree_to_file(file_path, etree_to_write):
        ...
        with open(file_path, "wb") as file:
            file.write(
                etree.tostring(etree_to_write, encoding="utf-8", pretty_print="true", xml_declaration=True)
            )
...
def main():
    ...
    #migration.validate() disabled as parameters cause validation error in OpenSCENARIO <1.0
